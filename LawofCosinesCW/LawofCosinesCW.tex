...
\section{A Tricky Triangle}

\begin{question}  \label{QLEd3e99}
In $\Delta PQR$, side $\overline{PR}$ has length $q = 6$ cm, side $\overline{PQ}$ has length $r = 3$ cm, and  angle $\angle P$ has measure $\arccos(5/9)$. 

\begin{enumerate}
\item Find the exact length of side $\overline{QR}$. Do \emph{not} use a calculator.

\item Use the law of sines to find the exact measure of angle $\angle Q$. Do \emph{not} use a calculator.

\item Use the law of cosines to find the exact measure of angle $\angle Q$. Do \emph{not} use a calculator.

\item Do your answers in parts (b) and (c) agree? Explain why or why not without using a calculator. If not, which is correct?

\item Starting from scratch, how would you use the law of sines to find the measure of angle $\angle Q$?

\item Use the law of sines to find the exact measure of $\angle Q$. Do not use a calculator. Does your result agree with part (c)?
...
\begin{question} \label{Q00:Si44nfefeosine}
A tree leans precariously (or maybe impossibly) with its trunk making at an angle of $\alpha = \arcsin(1/3)$ radians with the ground. The bottom end of a 5-foot ladder is 12 feet from the base of the trunk and the top end rests against the trunk. 

\begin{enumerate}

\item Drag the slider $u$ in Line 2 in the exploration below to slide the ladder and approximate the distance from its top end ($T$) to the trunk's base ($A$) when its bottom end ($B$) is 12 feet from the trunk's base. Find all possibilities.

\item Use the law of cosines in $\Delta ABT$ to find the exact distance(s) from the top of the ladder to the base of the tree when its bottom end is $12$ feet from the trunk's base. Do this by completing the square. Do \emph{not} use the quadratic formula.
...
\href{https://www.desmos.com/calculator/1dj0d53pq5}.
}
 
\begin{onlineOnly}
    \begin{center}
\desmos{1dj0d53pq5}{900}{600}
\end{center}
\end{onlineOnly}
\end{exploration} 

\href{https://www.desmos.com/calculator/1dj0d53pq5}{142: Ladder and Tree 2EB}
...
\href{https://www.desmos.com/calculator/a1lr8gkfgz}{142: Ladder and Tree 2B}
