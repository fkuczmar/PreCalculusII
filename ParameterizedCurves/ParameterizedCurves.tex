...
\noindent {\bf Solution:} The question is asking us to express the beetle's coordinates as functions of time, and we should start by defining time precisely. We'll let $t$ be the number of minutes past noon, so that for this problem $-2 \leq t \leq 20$. 

Now we need to find functions
\[
   x = f(t) \text{ and } y = g(t) , \text{ for } -2\leq t \leq 20 ,
\]
that express the coordinates of the beetle, measured in yards, in terms of the number of minutes past noon.
