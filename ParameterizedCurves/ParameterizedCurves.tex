...
\noindent {\bf Example 5:} Let $h,x_0>0$ be constants. Bug A leaves the point $(0,h)$ at time $t=0$ seconds and moves with constant speed $v$ m/s in the direction of the positive $x$-axis along the line $y=h$. Bug $B$ leaves the origin some time later and moves with a constant speed $w$ m/s directly toward the point $(x_0,h)$. The bugs eventually collide.

\begin{question}
 Find an expression for the time $t_0$ (measured in seconds since time $t=0$) of the collision. %in terms of $h$, $v$, and $w$ for the time $t_0$ of the collision.  
\[
