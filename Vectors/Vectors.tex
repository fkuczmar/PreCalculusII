...
\]   
%that the position vector $\overrightarrow{GE}$ of $E$ relative to $G$ is some \emph{positive} multiple of the vector $\overrightarrow{AQ}$ giving the position of $Q$ relative to $A$.
...
\begin{question} \label{QKDFMR33}
Point $B$ is $3$ miles due west of point $A$.

Point $C$ is $8$ miles from $A$ at a bearing of $2$ radians.

\begin{enumerate}
\item In a rectangular coordinate system with the positive $x$-axis pointing due east, find the \emph{exact} components of the vectors that gives the positions of 

\begin{enumerate}
\item $B$ relative to $A$.

\item $C$ relative to $A$.

\item $B$ relative to $C$.

Do  not use a calculator.

\item Use a calculator to approximate the components of the vector giving the position of $B$ relative to $C$. Then use the worksheet in Example 10 above to approximate the direction you would head to go from $C$ to $B$.

\item Find a simplified expression for the exact distance between $C$ and $B$. Use a calculator to approximate this distance. Use the worksheet in Example 10 to check your work.

\end{enumerate}
\end{enumerate}
\end{question}

\begin{question} \label{Q9IRErerr3e}
Point $A$ is 4 miles due east of  point $B$.

Point $C$ lies at a bearing of $0.9$ radians from $A$ and is 7 miles from $B$.

Use vectors (and not triangle trigonometry) to find the exact distance between $A$ and $C$. Start by defining an unknown. Use the worksheet in Example 10 to check your work.
