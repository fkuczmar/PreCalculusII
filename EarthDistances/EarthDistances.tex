...
We saw in Question 2 (a) above, that the distance (in miles) along their common circle of latitude is
\[
      d_1 = R \Delta \theta \cos\phi .
\] 

To compute the distance along the shorter arc of the great circle we need to see inside the earth. Do this by deactivating the \emph{Sphere} folder in Line 3.
...
The center of the great circle (blue) coincides with the center $O$ of the sphere. To compute the distance along the shorter arc of this great circle, we need to measure the angle $\alpha = \angle AOB$. The two-dimensional figure below can help.  
...
\item Use the two-dimensional figure above to find an expression for the measure $\alpha$ of $\angle AOB$. 

\emph{Hint:} Use right triangle $\Delta QMB$ to first find an expression for the length of $\overline{BM}$ in terms of $R$, $\phi$, and $\Delta \theta$. Then use right triangle $\Delta OMB$ to find an expression for $\alpha/2 = \angle MOB$.
