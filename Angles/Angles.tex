...
\begin{onlineOnly}
    \begin{center}
\desmosThreeD{qnygdqj2zg}{900}{600}
\end{center}
\end{onlineOnly}
...
\begin{enumerate}
\item How many radians are in $4.5$ revolutions?

\item How many revoultions do you make in turning through an angle of $20$ radians?

\end{enumerate}
\begin{explanation}
\begin{enumerate}
\item Since there are $2\pi$ radians in one revolution, there are 
\[
   (4.5 \text{ rev})  \left( \frac{2\pi \text{ rad}}{1 \text{ rev}}  \right)  = 9\pi \text{ radians} \sim 28.3 \text{ radians}
\]
in $4.5$ revolutions.

\item In turning through an angle of $20$ radians, you make
\[
    (20\text{ rad}) \left( \frac{1 \text{ rev}}{2\pi \text{ rad}}  \right) = \frac{10}{\pi} \text{ rev} \sim 3.18 \text{ revolutions} .
\]
\end{enumerate}
\end{explanation}
\end{question}
...
\begin{onlineOnly}
    \begin{center}
\desmosThreeD{qh9mexvcv0}{900}{600}
\end{center}
\end{onlineOnly}
...
\begin{onlineOnly}
    \begin{center}
\desmosThreeD{91881c3088}{900}{600}
\end{center}
\end{onlineOnly}
...
\begin{enumerate}
\item In which quadrant is the beetle after it stops? Click on the arrow below for the solution.

\begin{expandable}
The number of revolutions the beetle turns about the origin is
\[
    90 \text{ rad} \left( \frac{1\text{ rev}}{2\pi \text{ rad}} \right) = \frac{45}{\pi} \text{ rev} \sim 14.32 \text{ rev}.
\]
Since the beetle turns a bit more than $14.25$ revolutoions it stops in the second quadrant.
\end{expandable}

\item If the beetle wanted to crawl along the circle back to its starting point, in which sense (clockwise or counterclockwise) should it head to get there as fast as possible? How long would it take the beetle crawling at the constant speed of $4$ cm/sec?  Click on the arrow below for the solution.

\begin{expandable}
Since the beetle is in the second quadrant, it should crawl clockwise around the circle to return to the point $(20,0)$ as fast as possible. In doing so, it turns about the origin through the angle
\[
     \theta = 90\text{ rad} - 14\text{ rev} \left( \frac{2\pi \text{ rad}}{1 \text{ rev}} \right) = 90 - 28\pi \text{ rad} 
\]
and crawls through the distance
\[
 s =  (  90 - 28\pi \text{ rad}  ) \left( 20 \frac{\text{ cm}}{\text{ rad}} \right) = 20 (90 - 28\pi)\text{ cm} \sim 40.7\text{ cm}.
\]
To crawl this distance at a constant speed of $4$ cm/sec takes
\[
         \frac{20 (90 - 28\pi)\text{ cm}}{4\text{cm/sec}} = 5(90-28\pi)\text{ sec} \sim 10.18 \text{ sec}.
\]
\end{expandable}
...
\begin{question} \label{QOLfmdfrggfg}
You and your friend jog counterclockwise around a circular track at the respective speesds speeds of $3$ m/sec and $5$ m/s. You run in the inner lane, a circle with radius $40$ meters, while your friend runs in the outer lane, a circle with radius $44$ meters. How often do you pass each other?
\end{question}
