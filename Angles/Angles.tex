...
But it turns out that there is an even easier way to measure angles. %This way is suggested by the following problem.
...
This problem suggests a natural way to measure angles using \emph{radians}. The word \emph{radian} comes from radius and an angle measuring 1 radian placed with its vertex at the center of a circle subtends (cuts out) an arclength equal to the circle's radius. An angle with a measure of 2 radians similarly placed subtends an arclength equal to twice the circle's radius. 
...
To answer this question, draw an imaginary circle about yourself, say with radius $r$ meters. Then dividethe circumference of that circle by its radius. That dimensionless ratio gives you the radain measure of one revolution. It measures the number of radii that you need to wrap around the circle's circumference.

\begin{exploration}  \label{E33:Angles}
\pdfOnly{
Access Desmos interactives through the online version of this text at
 
\href{https://www.desmos.com/calculator/cc2vxblkw3}.
}
 
\begin{onlineOnly}
    \begin{center}
\desmos{cc2vxblkw3}{900}{600}
\end{center}
\end{onlineOnly}
\end{exploration}
...
The area $A$ of a spherical triangle on a sphere of radius $r$ with angle measures $\alpha$, $\beta$, $\gamma$ is given by the formula
\[
