...
\]
But this would \emph{not} be correct. The reason is that the arcsine function returns angles between $-\pi/2$ and $\pi/2$ radians. And because the vector $\overrightarrow{AB}$ has a negative $x$-component and a postive $y$-component, we know that its polar angle is between $\pi/2$ and $\pi$ radians.
