...
To do this, we first need to recognize that we're working on a circle of radius $r=5$cm. Now we'll let $P(x,y)$ be the point on this circle with polar angle $\theta = \arccos(1/4)$. To find where $P$ lies on the circle, we'll find the $x$-coordinate of $P$.
...
To find where $P$ is on the circle, we draw the line $x=1.25$. This line intersects the circle in two points, $P$ and $Q$, where $P$ is in the first quadrant and $Q$ in the fourth (see demonstration below). Now remember that the arccosine function returns angles bewteen $0$ and $\pi$. But since $Q$ is in the fourth quadrant, there is no way to measure its polar angle to be between $0$ and $\pi$. But there is such a choice for the point $P$ in the first quadrant. We can use the protractor estimate its polar angle to be about $1.3$ radians.
...
\href{https://www.desmos.com/calculator/pzftbzlfkn}.
