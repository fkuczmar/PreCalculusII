...
\begin{question} \label{QKDFer3rcv}
Which two of the following expressions are equal? Draw a picture of a circle to help with your explanation.

\begin{selectAll}
\choice[correct]{$\sin\theta$}
\choice{$\cos\theta$}
\choice{$\sin(-\theta)$}
\choice{$\cos(\pi - \theta)$}
\choice[correct]{$\sin(\pi-\theta)$}
\end{selectAll}
...
\item Drag point $R$ in the worksheet above to make triangle $\Delta PQR$ satisfy the above conditions. Then  drag the slider $r$ in Line 1 to approximate the lengths $p$ and $q$ of the respective sides $\overline{QR}$ and $\overline{PR}$.
...
\item Use right triangle trigonometry to find the exact length of sides $p=QR$ without using a calculator. \emph{Hint:} Activate the folder \emph{Triangle Solution} in Line 3 of the above worksheet.
...
\section{The Height of a Tree}

\begin{question} \label{QLOdfeEAAA}
You measure the angle of elevation to the top of a tree to be $\theta_1$ radians. From your current position, you then walk an additional  $s$ feet directly away from the tree and measure the angle of elevation to the top of the tree to be $\theta_2$ radians. Express the height of the tree above eye level in terms of $s$ and the angles $\theta_1$, $\theta_2$. Do this by first using the law of sines and then right triangle trigonometry.
