...
To move with \emph{uniform circular motion} means to move around a circle at a constant speed. The animation below shows an example where a beetle crawls counterclockwise around a circle of radius $20$cm centered at the origin.

\pdfOnly{
Access Desmos interactives through the online version of this text at
 
\href{https://www.desmos.com/calculator/uzyhbw8qw6}.
}
 
\begin{onlineOnly}
    \begin{center}
\desmos{uzyhbw8qw6}{900}{600}
\end{center}
\end{onlineOnly}
\end{exploration}

To parameterize the motion we need more information than just the path. We need enough information to determine the position of the beetle at any time.

So we'll assume that the beetle starts from the point $(20,0)$ at time $t=0$ seconds past noon and that it crawls at the constant speed of $4$ cm/sec. We'll also assume that the beetle crawls around the circle for two minutes. Our problem now is to express the coordinates of the beetle as functions of time, say $t$, measured in the number of seconds past noon. That is, we wish to find \emph{coordinate functions}
\[
...
So to parameterize the beetle's motion, where $r=20$ cm, we need only express the polar angle as a function of time.
